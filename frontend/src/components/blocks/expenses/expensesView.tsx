...
type DayGroup = { date: string; rows: Transaction[]; total: number };

function groupByDay(txs: Transaction[]): DayGroup[] {
  const map = new Map<string, Transaction[]>();
  for (const t of txs) {
    const arr = map.get(t.date.slice(0,10)) ?? [];
    arr.push(t);
    map.set(t.date.slice(0,10), arr);
  }
...
const ExpenseCard = ({ tx, onEdit, onDelete }: {
  tx: Transaction; onEdit: () => void; onDelete: () => void;
}) => {
  const def  = categoryFor(tx.category);
  const Icon = def.icon;
  const dateStr = new Date(tx.date + (tx.date.length === 10 ? "T12:00:00" : ""))
    .toLocaleDateString("en-PK", { day:"numeric", month:"short", year:"numeric" });
  return (
    <div className="group relative rounded-2xl border border-line-strong bg-surface overflow-hidden
      ...
      <div className="p-3">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="group-hover:hidden font-mono text-[9px] text-faint shrink-0">{dateStr}</span>
        </div>
...
const Tile = ({ label, value, sub, tone = "muted" }: {
  ...
  const variant =
    tone === "expense" ? "ff-tile ff-tile--expense"
    : tone === "warning" ? "ff-tile ff-tile--warning"
...
const ExpensesView = () => {
  ...
  const allExpenses = useMemo(
    () => (data ?? []).filter(t => t.type === "expense"), [data]);
  ...
  const topCat = useMemo(() => {
    ...
    let best = "", bestAmt = 0;
    map.forEach((amt,cat) => { if (amt > bestAmt) { bestAmt = amt; best = cat; } });
  ...
  const monthlyBreakdown = useMemo(() => {
    ...
    return Array.from({ length: 12 }, (_,i) => {
      ...
      return rows.length ? {
        month: mo, label: MONTHS[i].slice(0,3),
        total: rows.reduce((s,r) => s + Number(r.amount), 0),
        count: rows.length,
      } : null;
  ...
  const exportCsv = () => {
    ...
    const lines  = filtered.map(t =>
      [formatDate(t.date), t.category, JSON.stringify(t.description ?? ""),
       Number(t.amount).toFixed(2), JSON.stringify(t.wallet?.name ?? "")].join(","));
  ...
  return (
    <PageLayout
      eyebrow="Outflows"
      title={<>Track every <em className="italic text-terra">expense</em>.</>}
    ...
    >
      ...
      <div className="ff-toolbar">
        ...
        <div className="ff-toolbar__filters">
          ...
          <Input label="Min ₨" type="number" placeholder="0" value={minAmount} onChange={e => setMinAmount(e.target.value)} />
          <Input label="Max ₨" type="number" placeholder="Any" value={maxAmount} onChange={e => setMaxAmount(e.target.value)} />
        </div>
      </div>
      ...
      <div className="ff-tile-grid ff-tile-grid--3">
        ...
        <Tile label="Top category" value={topCat.name || "—"}
          sub={topCat.amount > 0 ? `₨ ${formatPKR(topCat.amount,{showSymbol:false})}` : undefined} />
      </div>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
            {monthlyBreakdown.map(({ month, label, total, count }) => (
              <button key={month} type="button"
                ...
                <p className="font-mono text-[10px] tracking-widest uppercase text-muted group-hover:text-primary transition-colors">{label}</p>
      ...
      <Modal open={addOpen} onOpenChange={setAddOpen}
        title="Add expense" description="Log a new expense. Wallet balance will update automatically.">
        <TransactionForm defaultType="expense" lockType onDone={() => setAddOpen(false)} />
      </Modal>

      <Modal open={!!editing} onOpenChange={v => !v && setEditing(null)}
        title="Edit expense" description="Update the expense details below.">
        {editing && <TransactionForm defaultType="expense" lockType editing={editing} onDone={() => setEditing(null)} />}
      </Modal>

      <Modal open={!!deleting} onOpenChange={v => !v && setDeleting(null)}
        title="Delete expense?" description="This cannot be undone.">
        {deleting && (
          <div className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <Button variant="ghost" onClick={() => setDeleting(null)} disabled={del.isPending}>Cancel</Button>
              <Button variant="danger" loading={del.isPending} onClick={() => void confirmDelete()}>Delete</Button>
            </div>
          </div>
        )}
      </Modal>
